...
class Table extends React.Component<ITableProps, ITableState> {
  ...
  public componentDidUpdate(prevProps: ITableProps, prevState: ITableState) {
    ...
    if (onSelectChange) {
      ...
      const isEqual = isSimpleArrayEqual(selectedKeys, prevSelectedKeys)
      if (!isEqual) {
  ...
  public renderMaskedLeftFixedHead = (renderDataSource: IData[]) => {
    const cn = componentName
  ...
  // 渲染右侧固定表头
  public renderRightFixedHead = (renderDataSource: IData[]) => {
    const cn = componentName
  ...
  // 表头选择框类名
  public getSelectionClassName = (renderDataSource: IData[]): string => {
    const { selectedKeys } = this.state
    const renderKeys = renderDataSource.map(data => data.key)
    const isEqual = isSimpleArrayEqual(selectedKeys, renderKeys)
  ...
  // 监听点击表头选择框
  public handleClickHeadSelection = (renderDataSource: IData[]) => {
    const { selectedKeys } = this.state
    const renderKeys = renderDataSource.map((data: IData) => data.key)
    const isEqual = isSimpleArrayEqual(selectedKeys, renderKeys)
    if (isEqual) {
  ...
  public renderBody = (renderDataSource: IData[]) => {
  ...
  // 渲染 row 中的列
  public renderBodyRowCell = (columns: IColumn[], data: IData) => {
    const cn = componentName
    const { size, border } = this.props
  ...
  public getRenderDataSource = () => {
    ...
    // 无 sorter 升序降序
    if (!sorter) {
      return dataSource.sort((rowA: IData, rowB: IData) => {
        // 升序
        if (sortOrder === 'ascend') {
  ...
  public renderMaskedLeftFixedBody = (renderDateSource: IData[]) => {
    const cn = componentName
    const { columns, scroll, selectCol } = this.props
    const { headHeight, xScrollPosition } = this.state
    const leftFixedColumns = columns.filter(col => col.fixed === 'left')
  ...
  // 渲染右侧固定列
  public renderRightFixedBody = (renderDataSource: IData[]) => {
    const cn = componentName
